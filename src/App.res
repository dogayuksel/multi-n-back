...
let reducer = (state: state, action: action): state =>
  ...
  | UpdateModalityConfig(modality, value) => {
      ...state,
      config: state.config |> GameConfiguration.updateModality(modality, value),
    }
  | ToggleConfigPanelOpen => {
      ...state,
      configPanelOpen: !state.configPanelOpen,
    }
  | UpdateAnswer(answer) => {...state, answer: answer}
  | AdvanceTurn =>
    let partiallyUpdatedState = if state.stateHistory->List.length >= state.config.depth {
      switch GameState.compareToHistory(
        state.answer,
        state.gameState,
        state.stateHistory,
        state.config,
      ...
      | Some(result) => {
          ...state,
          stateHistory: list{state.gameState, ...state.stateHistory},
          score: Score.calculateScore(result, state.config.depth) + state.score,
        }
      | None => {
          ...state,
          stateHistory: list{},
...
let make = () => {
  ...
  let toggleAnswer = modality => dispatch(UpdateAnswer(state.answer |> Answer.toggle(modality)))
  ...
  <>
    ...
    {!state.configPanelOpen
      ? <div style={ReactDOMRe.Style.make(~display="flex", ~justifyContent="center", ())}>
          <button
            ...
            {List.length(state.stateHistory) == 0 ? React.string("Start") : React.string("Next")}
          </button>
        </div>
      : React.null}
    {!state.configPanelOpen && List.length(state.stateHistory) == 0
      ? <ConfigurationTrigger toggleConfigPanelOpen={_ => dispatch(ToggleConfigPanelOpen)} />
      : React.null}
    ...
    <ConfigurationPanel
      ...
      config=state.config
      updateModalityConfig={(modality, value) => dispatch(UpdateModalityConfig(modality, value))}

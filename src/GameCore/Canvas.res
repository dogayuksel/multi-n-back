...
let make = (~config: GameConfiguration.t, ~gameState: GameState.t) => {
  let modalityIndices = Modality.allModalityTypes |> Array.fold_left((acc, modality) => {
    let value = switch (config.modalities |> getValue(modality), gameState |> getValue(modality)) {
  ...
  let positionDepth = switch config.modalities |> getValue(Position) {
  | Some(positionDepth) => positionDepth
  | _ => 1
  }

  let wrapperStyles = ReactDOMRe.Style.make(
    ~display="grid",
    ~alignItems="center",
    ~justifyItems="center",
    (),
  ...
  <div
    ...
    |> Array.mapi((renderIndex, _) => {
      let active = modalityIndices |> Modality.getValue(Position) == renderIndex
      <Color
        width
        height
        key={string_of_int(renderIndex)}
        active={active && config.modalities |> Modality.getValue(Color) |> Belt.Option.isSome}
        index={modalityIndices |> Modality.getValue(Color)}>
        <Icon
          active={active && config.modalities |> Modality.getValue(Icon) |> Belt.Option.isSome}

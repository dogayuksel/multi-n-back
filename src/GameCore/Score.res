...
@val
external setInLocalStorage: (~key: string, ~value: string) => unit = "window.localStorage.setItem"
...
let calculateScore = (result: GameState.result, depth: int): int =>
  Modality.allModalityTypes |> Array.fold_left((acc, cur) =>
...
let getHighScore = (): option<int> => getHighScore()->Belt.Option.flatMap(int_of_string_opt)

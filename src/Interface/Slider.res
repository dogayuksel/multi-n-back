...
let make = (~label: string, ~value: option<int>, ~onChange: int => unit) => {
  let intValue = switch value {
  | Some(number) => number
  | None => 0
  }
  <div style={ReactDOMRe.Style.make(~margin="12px", ~display="flex", ~alignItems="center", ())}>
    <div style={ReactDOMRe.Style.make(~minWidth="60px", ~textAlign="right", ~margin="12px", ())}>
      {React.string(label)}
    </div>
    ...
    <div
      style={ReactDOMRe.Style.make(
        ...
        ~height="20px",
        ~borderRadius="2px",
        ~boxShadow="inset 4px 4px 12px " ++
        (AppStyles.background_more_darker ++
        (", inset -4px -4px 12px " ++ AppStyles.background_less_lighter)),
      ...
      <div
        style={ReactDOMRe.Style.make(
          ...
          ~opacity="0.7",
          ~borderRadius="2px",
          ~boxShadow="inset 1px 1px 3px " ++
          (AppStyles.background_more_darker ++
          (", inset -1px -1px 3px " ++ AppStyles.background_less_lighter)),
          (),
        )}
      />
    </div>
